...
export async function POST(request: Request) {
  try {
    const { idea, platforms }: GenerateRequest = await request.json();

    if (!idea || !platforms || platforms.length === 0) {
      return NextResponse.json(
        { error: 'Idea and platforms are required' },
        { status: 400 }
      );
    }
    ...
    const generatedContent: GeneratedContent = {
      tiktok: {
        hooks: generateTikTokHooks(idea),
        captions: generateTikTokCaptions(idea),
      },
      instagram: {
        bio: generateInstagramBio(idea),
        captions: generateInstagramCaptions(idea),
        hashtags: generateInstagramHashtags(idea),
      },
      linkedin: {
        posts: generateLinkedInPosts(idea),
      },
    };
    ...
    if (platforms.includes('tiktok')) {
      responseContent.tiktok = generatedContent.tiktok;
    }
    
    if (platforms.includes('instagram')) {
      responseContent.instagram = generatedContent.instagram;
    }
    
    if (platforms.includes('linkedin')) {
      responseContent.linkedin = generatedContent.linkedin;
    }
...
function generateTikTokHooks(idea: string): string[] {
  const hooks = [
  ...
  return shuffleArray(hooks).slice(0, 3);
}

function generateTikTokCaptions(idea: string): string[] {
  const captions = [
  ...
  // Return 2 random captions
  return shuffleArray(captions).slice(0, 2);
}

function generateInstagramBio(idea: string): string {
  const bios = [
...
function generateInstagramCaptions(idea: string): string[] {
  const captions = [
  ...
  return shuffleArray(captions).slice(0, 3);
}

function generateInstagramHashtags(idea: string): string[] {
  const baseHashtags = [
  ...
  return shuffleArray(baseHashtags).slice(0, 8);
}

function generateLinkedInPosts(idea: string): string[] {
  const posts = [
  ...
  return shuffleArray(posts).slice(0, 2);
}
